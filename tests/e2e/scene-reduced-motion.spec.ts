...
// Playwright cannot import `src/data/archive` (`embeds.csv?raw` is Vite-only),
// so the row count comes from the frieze fixture — one cell per archive item,
// regenerated by a unit test that fails on drift.
const ROWS = (
...
test('reduced motion keeps the pin and swaps cards without flight', async ({ page }) => {
  ...
  // Reduced motion changes how the scene MOVES, never what it contains: the
  // stream is the same 171 rows here as anywhere.
