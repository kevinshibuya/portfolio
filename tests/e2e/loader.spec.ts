...
test('loader shows the ks. window mark + corner meta, explodes away, zero console errors', async ({ page }) => {
  const errors: string[] = []
  ...
  await expect(page.locator('#loader mask#loader-mask g.loader-ks')).toHaveCount(1)
  await expect(page.locator('#loader .loader-meta')).toHaveCount(2)
  await expect(page.locator('#loader')).toContainText('portfolio · 2026')
  // Explosion completes → loader removed, gate resolved.
...
test.describe('reduced motion', () => {
  ...
  test('reduced motion: loader fades (no explosion) and is removed', async ({ page }) => {
    await page.goto('/')
