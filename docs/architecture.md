...
The harness's own e2e specs are **dormant** in this tree. `tests/e2e/pixel-gate.spec.ts`, `tests/e2e/perf-hooks.spec.ts` and the Layer 1 half of `tests/e2e/perf-budget.spec.ts` skip unless `PERF_HARNESS=1`, because their baseline was recorded against the August site and parts of it no longer describe what this tree renders. The two ungated tests in `perf-budget` (CLS, long tasks) keep running by default. Re-baselining follows ADR 0006 and 0007 · on the rig, on measured evidence.
