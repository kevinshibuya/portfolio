...
Commands: `npm run perf` runs everything, `npm run perf:lh` Layer 3 alone, `npm run perf:selftest` proves the runner itself. The reference numbers live in `perf/baseline.json` under `rig`, `exact`, `scenarios` and `lighthouse`; kept wins ratchet it down so later batches cannot give them back. A batch is one hypothesis, kept or reverted on the measurement alone · no partial credit and no unmeasured "should help elsewhere" argument (ADR 0007).

The harness's own e2e specs **run by default**, with four exceptions. What runs is self-certifying: `tests/e2e/perf-hooks.spec.ts` asserts the instrumentation contract, three Layer 1 tests in `tests/e2e/perf-budget.spec.ts` assert exact per-frame GL work, and 24 goldens in `tests/e2e/pixel-gate.spec.ts` still match this tree, so ADR 0007's "regenerate only on a commit that declares visual intent" is enforceable · an intentional hero edit now carries `--update-snapshots` in that commit. Four tests skip unless `PERF_HARNESS=1`: `stage-arrival-t2` (its golden is of the August DOM stack, the `.stack-card` it counts no longer exists under ADR 0011, and the scene has no `?perf-freeze` hook, so no deterministic stage golden can be baked yet), the capped-DPR backing-store contract (the scene is a third canvas and the contract is undecided), the chunk byte ceiling (a measured number, re-baselined on the rig), and `perf-counters` (a throughput assertion that `pixel-gate` starves when it runs first). Issue #11 carries all four.
