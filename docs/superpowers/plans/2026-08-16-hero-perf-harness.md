...
- [x] **Step 5 (REVISED, Kevin 2026-08-31): establish the MINIMUM DETECTABLE EFFECT, not a pass/fail on one plant.** The original bar — "the harness must flag `i<5` -> `i<10`" — rests on an assumption measurement killed: that plant was believed to roughly double shader cost, and at the rig's real scale it is a **+2.6% effect** (+0.1554 ms on a 5.8842 control mean). It only looked like +49% at `deviceScaleFactor: 1`, where a 1.3 Mpix canvas is dominated by the loop; the rig's 2.9 Mpix canvas is not. That bar is harsher than anything this campaign will attempt. Instead: run a **plant sweep** (`i < 10` / `i < 20` / `i < 40`) with control legs **INTERLEAVED** (C P C P C P C, or randomised order) — non-negotiable, because the controls drift monotonically upward across legs (5.8348 -> 5.8868 -> 5.9310 observed) and a block design lets that drift masquerade as signal. Report the smallest plant the runner separates from the control band, and PROVE that MDE sits below Task 8's expected ~23% win (~1.35 ms, ~14x the 0.0962 three-control spread). Record the MDE as the campaign's stated detection threshold; any batch claiming a win smaller than it is not decidable by this harness and must say so. **Outcome: RUN, and the plant ladder turned out NOT to be a ruler.** Seven legs, `32f1eab`, 2026-09-01: `i<10` measured −4.40%, `i<20` +5.86%, `i<40` +79.18% — non-monotonic, three differently-compiled shaders rather than three points on one curve, so `i<N` is NOT a proxy for N/5 times the shader cost. What the sweep DID calibrate is the instrument: four interleaved controls spread 0.0323 ms (0.48%) across 26 min with identical dist hashes. MDE under the default band `i<40` (+5.31 ms, decides nothing); under the post-5b band `i<20` (**+0.3929 ms, +5.86%**). **Stated detection threshold: ~0.1 ms (~1.5%).** Task 8's ~1.35 ms win sits ~13x above it — decidable. Full table in `perf/decisions.md`.
- [x] **Step 5b:** From the sweep's control legs, set a **`maxBand` override** for `gpu.shaderMsPerFrame` in `baseline.json` (`stats.mjs:39`, the same mechanism `fc7bcca` used for Lighthouse from 32 runs of measured spread). It is REQUIRED: `bandFor` is `max(10% of median, 1x IQR, minBand)` and 10% of 5.89 = 0.5887 swamps an observed IQR of 0.0403-0.1114, so the default band hides every effect this metric can actually see. Retire the interim `minBand: 0.37` (the single-run floor) in the same edit — it currently sits above the plant and below the 10% term, so it does nothing. **Outcome: `"maxBand": 0.1` set on idle-hero `gpu.shaderMsPerFrame` in `baseline.json`, `minBand: 0.37` retired from `idle-hero.mjs`; scroll-transition untouched, per the ruling's idle-hero-only clause. Derivation: 3x the 0.0323 control-median range = 0.0969 vs 2x the 0.030 largest IQR = 0.060, ceiled to 0.10. `npm run perf:selftest` 29/29 + 57/57, exit 0. OPEN: only 0.0145 ms of headroom against the standing baseline median — see `perf/decisions.md`.**
  **Step 5 / 5b design ruling (fable session, approved by Kevin 2026-09-01) — execute exactly this, in a fresh opus session:**
  1. **Driver.** A new committed script `perf/mde-sweep.mjs` spawns `node perf/run.mjs idle-hero --runs 5` as a child process through the public CLI and reads the report JSON each invocation writes to `perf/reports/`. It does NOT import runner internals — the MDE must be the number `npm run perf` itself produces. Exit 1 from a plant leg is the wanted signal, not a failure; exit 2 or a crash aborts the sweep after the revert.
  2. **Legs.** Seven, `C P10 C P20 C P40 C`, fixed ascending plant order (reproducible; with three plants randomisation adds nothing over adjacent-control pairing). Plant = `for (int i = 0; i < 5; i++)` -> `i < N` in `FluidWaves.tsx`, verified in the served bundle by grep on `dist/assets` (as `gpu-timer-probe.mjs aba` does). `--runs 5` because the MDE must be measured at the gating configuration.
  3. **Verdict.** Plant k is SEPARATED iff `median(Pk) - mean(median(C_k), median(C_k+1)) > band`, where band = the runner's own arithmetic (`applyBandOverrides(bandFor(median, iqr, minBand), overrides)`) applied to the WIDER of the two adjacent controls (mirrors `--compare`). The runner's verdict against `baseline.json` is reported alongside as a second column. If the two disagree at the MDE plant, the MDE is the LARGER plant.
  4. **Collect once, evaluate twice.** Reports already carry median/iqr/band + dist content hash per leg; the sweep keeps all seven report paths and evaluates the table analytically under BOTH the default band and the proposed `maxBand`. No re-run. `perf/reports/` is gitignored, so the full table goes verbatim into `perf/decisions.md` (Step 7).
  5. **Commit order.** Script (including the `maxBand` rule below) -> run -> commit results. Numbers without their script get retracted.
  6. **`maxBand` rule, fixed BEFORE any number exists:** idle-hero `gpu.shaderMsPerFrame` `maxBand` = the LARGER of (a) 3 x (max - min) of the four control medians and (b) 2 x the largest within-leg control IQR, rounded UP to 0.01 ms. Retire `minBand: 0.37` in the same edit. Idle-hero ONLY — scroll-transition's within-leg IQR (0.1514) is about the size of the expected band, so it keeps the default and the record says why.
  7. **Rig checks are hard aborts inside the sweep.** Power logged at both ends of EVERY leg; a battery reading anywhere aborts after the revert. Load guard sampled before each leg with a few retries over ~3 min (transient `PerfPowerServices`/`coreduetd` spikes). `caffeinate -disu -t 10800` re-armed first.
  8. **Revert is unconditional.** Assert `git diff --stat src/` empty before leg 1; revert on normal exit, any throw, SIGINT, SIGTERM; assert empty again at the end.
  9. **Control build hashes must match.** The four control legs must produce an identical dist hash; if not, the build is nondeterministic and the sweep says so before any delta is read.
  10. **Expected outcome, stated ahead:** default band ~0.66 ms vs a ~0.10-0.17 ms `i<10` plant, so MDE likely lands at `i<20` or `i<40` under the default band and at `i<10` under `maxBand`; Task 8's ~1.5 ms win sits above either. `i<40` failing under the default band is a result, not a bug. The probe's missing rig check stays an open gap: no probe-derived number enters the MDE.
- [x] **Step 6:** Re-record `scenarios` in `perf/baseline.json` after a warm-up, per the ruling's second half. The first-invocation-reads-low effect (recorded 2026-08-24, mechanism still unidentified) is untouched by the new timer — the warm-up protocol must be stated and followed, and the determinism proof re-run.
- [x] **Step 7:** Record the whole thing in `perf/decisions.md` (route chosen + why the ruling's extension was superseded, the 8.4x mismeasurement, the A-B-A tables, the instrumentation-effect comparison); tick these boxes as each step lands. **Outcome: written 2026-09-01 as the `Task 7b (BLOCKER 2)` section of `perf/decisions.md` — Route C and why the 2026-08-24 extension ruling was superseded by measurement, the 8.4x mismeasurement reproduced at harness level, the instrumentation effect and why Step 3's own vsync-pinned test could not see it (the +51.9% timer term), both A-B-A tables, the seven-leg sweep verbatim, and the open items.**

**Acceptance (REWORDED 2026-09-01 on Kevin's ruling; this is what unblocks Tasks 8-12) — MET:** the original bar read "`npm run perf` detects the doubled-shader plant as a REGRESSION on `gpu.shaderMsPerFrame`", and Step 5's sweep proved that bar unsatisfiable for a reason that has nothing to do with the harness: on this rig the doubled plant (`i<10`) measures **4.4% FASTER** than the controls bracketing it. The harness detects that move and signs it correctly, as an improvement. The plant ladder also turned out to be non-monotonic (`i<10` −4.40%, `i<20` +5.86%, `i<40` +79.18%), so no `i<N` is a proxy for N/5 times the shader cost and none of them can calibrate an effect size. The bar is therefore the one Step 5's own revision already implies, in three parts, all evidenced in `perf/decisions.md`: **(a)** the harness separates a plant from its bracketing control legs at the campaign's stated detection threshold — **~0.1 ms (~1.5%)**, the `maxBand` derived in Step 5b from four interleaved control legs spreading 0.0323 ms across 26 minutes — demonstrated at `i<20` (**+0.3929 ms, +5.86%**); **(b)** the control legs agree within that band, all four of them, on an identical `dist` hash; **(c)** the threshold sits below the campaign's expected wins — Task 8's ~1.35 ms is ~13x above it. Any batch claiming a win smaller than 0.1 ms on this metric is **not decidable by this harness and must say so in its decision line**. Tasks 8-12 are unblocked.
