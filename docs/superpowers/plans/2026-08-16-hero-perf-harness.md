...
- [ ] **Step 5:** Re-run the sensitivity proof **through the real runner** (`npm run perf`), not just the probe: plant `FluidWaves.tsx:158` `i < 5` -> `i < 10`, verify the plant is in the SERVED bundle, A-B-A, revert. The harness itself must flag it. Probe-level evidence is viability, NOT this acceptance — and at the rig's real scale a single-run probe CANNOT resolve this plant: A1 5.3681 / B 5.8895 / A2 5.7370 (+0.3369 ms, +6.1%) against an unplanted leg-to-leg floor of 0.3672 ms. Median-of-N through the runner is what has to close that gap. If it cannot, escalate the plant (`i < 20`) to establish the instrument's minimum detectable effect and record THAT as the campaign's detection threshold — Task 8 expects ~23%, well above the 6.5% floor.
