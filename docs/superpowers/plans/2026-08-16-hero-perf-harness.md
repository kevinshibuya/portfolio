...
- One loop per canvas: hero `rafLoopStarts === 1` after settle (the draw/uniform ratios alone cannot detect a duplicate loop driving the same drawFrame).
