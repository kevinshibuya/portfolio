...
**Acceptance check:** `npx vitest run tests/unit/navTarget.test.ts` → green on: `columnsFromSvh(1350) === 26`, `columnsFromSvh(700) === 0`, `columnsFromSvh(NaN) === null`, `columnsFromSvh(1360) === null` (not a whole column); `resolveNavTarget('archive', doc, 900)` on a stub document whose `.scene-scroll` has `data-svh="1350"`, `offsetHeight` 12150 and a known `getBoundingClientRect().top` equals `scrollTargetFor(volumeShotPlayhead(26), wrapperTop, 12150, 900, 26)` exactly; `resolveNavTarget('archive', docWithoutWrapper, 900)` is `'#archive'`; `resolveNavTarget('archive', docWithWrapperButNoSvh, 900)` is `'#projects'`; `resolveNavTarget('work', …)` is `'#work'`. The e2e is Task 10 test 5.
