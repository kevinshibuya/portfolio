...
**Corrected 2026-09-10, on the grep's scope.** "No fraction scroll survives in the files this task touches" is too broad as written: `light-chapter` and `nav-on-light` also scroll into `#work`, `#stats`, `#skills` and `#contact` by fraction, and those are ordinary sections where a fraction means exactly what it says. The hazard is the SCENE WRAPPER, which is `sceneWrapperSvh(35)` = 1575 svh tall. Only three calls were on it — `scrollIntoSection(page, 'projects', 0.4)` in `nav-on-light`, where 0.4 now lands deep in act two's dolly rather than 'the projects section' — and those became `scrollToPlayhead(page, 0)`. The acceptance grep is therefore `grep -n "scrollIntoSection(page, 'projects'"` over the six files, not a blanket fraction grep.

**Every scroll of the SCENE uses pipeline 1's `tests/e2e/helpers/scene.ts`** — `openScene`, `scrollToPlayhead(page, playhead)`, `scrollToActTwo(page, u)`, `readSvh(page)`, `CANVAS`. No fraction scroll survives in the files this task touches: the wrapper is now `sceneWrapperSvh(columns)` tall, so a fraction that used to mean "card two" now means "somewhere in the dolly", and a fraction-based assertion is silently testing a different beat than its name says. `scrollToPlayhead` derives its unit from `wrapper.offsetHeight / Number(wrapper.dataset.svh)`, which is exact at any column count.
...
- [x] Extend the six files
- [x] Kill 4173, run the six specs, green on both projects; commit `test(e2e): the stream joins the chapter's assertions`
...
**Corrected 2026-09-10, three ways.** (a) The `Consumes` line asks for `archive` and `yearBlocks` from
`src/data/archive`; Playwright cannot import that module (`embeds.csv?raw`, via `embeds.ts`, is Vite-only —
the fixture's own docstring says so). `frieze-cells.json` supplies both: `extent.blocks` IS `yearBlocks`
and `cells.length` is `archive.length`, and `playheadForItem(id, layout, extent)` is
`playheadForColumn(cell.col + cell.span / 2, extent)` by definition, so the fixture cell stands in for the
layout too. (b) Test 4's "focus parks the camera on the row's cell" holds only for a row that CROSSES a
year boundary — assumption 27 gates every other row, and the plan's own middle case sat 1350 px (6 columns)
off because of it. The cases are therefore the rows that OPEN a block. (c) Test 10's "the target for the
row that holds focus at the end" has the same problem, so the burst is aimed to END on a block's first row,
which the gate cannot suppress. Test 9's "`scrollY` did not move on the way out" is also wrong as written:
leaving the stream focuses a real element far up the document and the BROWSER scrolls to it — what must not
happen is a camera travel, so the assertion is that the newly focused element is the thing now in view.
...
4. **Focus moves the camera, and the assertion is the camera, not the attribute.** `data-focus-target` is written by the same handler under test, one line before the scroll, so asserting it proves only that the handler ran. This test asserts the APPLIED position. Setup once per case: `openScene`, `scrollToActTwo(page, 0)` (the volume shot), read `columns = columnsFromSvh(await readSvh(page))`, build `layout = friezeLayout(archive, rows)` and `extent = friezeExtent(layout, rows)` in Node, and read `wrapperTop` and `wrapper.offsetHeight` from the page.
   ...
   Assert: `window.scrollY` settles within 4 px of `scrollTargetFor(playheadForItem(id, layout, extent), wrapperTop, wrapperHeight, innerHeight, columns)` within 4000 ms; the canvas `data-act` is `'2'`; and the focused element is the pill — its box has `top === 16` and `clip-path` computes to `none` (the row is `absolute` inside the fixed `#archive` container; asserting `position === 'fixed'` on the row would fail by design), ~~and **`document.elementFromPoint(cx, cy)` at the pill's centre is the pill itself**~~ **Corrected 2026-09-10: not satisfiable, and the opposite is the invariant.** The pill inherits `pointer-events: none` by design, so the canvas keeps its hover — and `elementFromPoint` skips such elements, returning whatever is behind. Test 12 asserts what IS true: `pointer-events` computes to `none` and the hit at the pill's centre is NOT the pill. The stacking guarantee it was reaching for is carried instead by test 12's z-index comparison (`#archive` 120 > `header.nav` 100) and its ancestor scan, which is the mechanism that would actually break.
...
**Acceptance check:** `lsof -ti:4173 | xargs -r kill -9; npx playwright test tests/e2e/stream.spec.ts` → ~~`24 passed`~~ **`21 passed, 3 skipped`, corrected 2026-09-10.** Three skips, all designed in: test 3's `personal` half on both projects (no `personal` piece exists yet, spec decision 9 — it re-arms on the first one that lands), and test 5 on `mobile-chromium`, because `.nav-links` is `display: none` under 720 px and there is no mobile menu, so the `#archive` nav link is a desktop affordance by design. Test 6 keeps its row-focus half on phones and gates only its nav-link half.
