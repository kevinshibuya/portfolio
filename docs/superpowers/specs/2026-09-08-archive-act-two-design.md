...
- Not one canvas per cell. The existing caption path (one 2D canvas, one texture and two meshes per card, no instancing) does not scale to 171. Pipeline 2 rasterises **one coverage texture per year block** (the title's own technique, `titleTexture.ts`) at the density the dolly needs, up to a ceiling of **612.8 texels per world unit**, and capped at 4096 px per texture. A block whose mask would exceed the cap is drawn as **column-aligned panels**, one texture and one mesh each, split only on a column boundary and never through a 2×2 span; at six rows the 2024 block is two panels of eleven columns. The cap therefore never reduces raster detail; the ceiling does, uniformly, above 1080 CSS px of viewport height. It colours the mask in a shader from a per-cell colour lookup (origin tint, hover tint). One mesh per panel — a panel is a block, or a column-aligned slice of one; hover hit-testing is UV to cell, not a raycast over 171 meshes.
- The mask encodes text role in its channels so the shader colours each role from the per-cell lookup; the lookup texture's colour space is stated and asserted; masks carry no mipmaps. **Two channels, `RGFormat`:** R is title coverage, G is meta AND serial together, because the shader draws both in the same fixed muted ink and hover changes only the title — so they never need separating, and the pair must be asserted never to overlap within a cell. This resolves the earlier bullet's contradiction between "role in its channels" and "single-channel", and supersedes the RGBA8 reading in plan 2's Assumption 19. Two bytes per texel rather than four is what pays for the 612.8 ceiling: 27.2 MiB steady and 71.4 MiB peak, under the 86.45 MiB the plan review approved at RGBA8. A failed rasterisation degrades to a blank cream wall with `data-frieze="failed"` and never marks WebGL unavailable.
...
- [x] `feat/act-two-wall` merged into `feat/act-two`.
