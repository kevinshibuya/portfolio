...
Two consequences the first version of this note omitted. `titleCapPx` blends on the same `t`, so every viewport narrower than 800 px inside the band changes title size · up to 8.8 px, and 66.9 to 58.5 px on a 600×600 window. A 33-device sweep found no real device affected, because none sits inside the band. And the band fixes near-square only from about aspect 0.945 up: below the band start everything is at `t = 0`, so an 820-wide window still puts the shadow out of frame from 0.945 down to 0.70, worst 1.045 at 820×920 · which is worse than the 1.029 that prompted this change. An integer sweep of 600-1440 by 600-1200 windows says the retune removes about 27,400 overflowing sizes and adds none, so it strictly improves, but 54,400 remain.
