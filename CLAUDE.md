...
- **Accent usage**: Tricolor is applied via `accentFor(index)` (`src/utils/palette.ts`, `ACCENTS = ['#E64D66','#4D80E6','#E6CC4D']`, index-rotated) as a per-row `--row-tint` CSS var. Per-ROW tints always rotate; a section-level mark may pin a static accent (the Selected Work eyebrow numeral, `.section-index`, the nth-child `.skills-dot`), but an individual component never picks a colour for itself.
- **Canvases (three mounted, at most two live)**:
  ...
  - `SelectedWorkScene` (`src/components/canvas/SelectedWorkScene.tsx` + `src/components/canvas/scene/*`) — the THIRD canvas, and the only one rendered by React Three Fiber. Lives in the Projects lazy chunk; `data-canvas="selected-work-scene"`. Same shared rules as the other two (DPR ≤ 1.5, IntersectionObserver pause, `data-static` under reduced motion, permanent DOM fallback on context loss) — but the attributes go on the REAL canvas (`gl.domElement`) in `onCreated` and the visibility effects, never as `<Canvas>` props, which R3F forwards to its wrapper div. Pausing is `frameloop` `'always' ↔ 'never'`; reduced motion is `frameloop="demand"` plus `invalidate()` on scroll and resize. It also warms at idle after the entrance (`compileAsync`, `initTexture`, then ONE off-screen frame) and flags `data-warm="true"` — without that the first live frame costs ~0.5–1.2s on real hardware, right as the reader arrives. Depth of field and grain mount on desktop AND hardware-accelerated GPUs only; a software rasteriser skips the composer, so headless Playwright never renders it and `tests/e2e/scene-effects.spec.ts` spoofs a hardware renderer string to keep that path covered.
...
- **Selected Work scene (`src/components/sections/Projects.tsx` + `src/components/canvas/SelectedWorkScene.tsx` + `src/components/canvas/scene/*`, helpers `src/utils/sceneMotion.ts`)**: the page centerpiece — a real 3D environment in the third canvas, replacing the retired card stack and SVG gooey title. Spec `docs/superpowers/specs/2026-09-03-selected-work-scene-design.md`, plan `docs/superpowers/plans/2026-09-03-selected-work-scene.md`; ADR 0009 (third canvas via R3F) and ADR 0010 (scroll is the playhead, time is the breath).
  - **Anatomy**: `section#projects.section.projects-scene-section` › `nav.scene-skiplinks`; `div.scene-scroll` (450svh, the `useScroll` target) › `div.scene-sticky` (100svh, pinned) › `div.scene-inner` (carries `--row-tint`/`--row-tint-deep`) › `div.scene-canvas-wrap[aria-hidden][data-ready]`, `p.scene-eyebrow` › `span.scene-eyebrow-num`, `h2.scene-title-sr.sr-only`, and `div.scene-meta` (the front-card overlay: `.scene-meta-name`, `.scene-meta-subtitle`, `a.scene-meta-pill`). `div.scene-fallback` replaces `.scene-scroll` entirely when WebGL2 is missing or the context is lost — four framed cards in normal flow, no pin, permanent for the session.
  - **Corridor / slot / approach / playhead** (glossary in `CONTEXT.md`): the four featured projects (`highlightOrder ≤ 4`) stand along a corridor in depth, alternating side and yaw; scroll dollies the camera through it. `playheadFor(p) = p·3.5 − 0.5 ∈ [−0.5, 3]`: the leading 50svh is the **approach** (the camera eases in from one spacing back while card 0 surfaces from the fog and the title resolves from blur), then one viewport per card. Every integer playhead is a settled state and the whole thing is exactly reversible.
  - **Zero React state per frame**: ONE `useFrame` in `SceneRig` reads Framer's `scrollYProgress`/`useVelocity` and the clock, and writes every visual through the pure helpers in `sceneMotion.ts`. `frontIndex` flips only at settle midpoints and feeds ONLY non-visual attrs (the overlay `<Link>`, aria, the SR heading, the row tints); the loop derives its own visual index and never reads it.
  - **Title**: Anton on a camera-relative plane (NOT parented to the camera — R3F's default camera is outside the scene graph), two coverage-mask textures blurred by mip LOD, added, then cut at a hard threshold to reproduce the gooey bridge. It is the one object the cream fog never touches, and the only Anton consumer on the site. The band is **bottom-anchored**: its lowest ink sits just above the settled card's top edge and grows upward, because the card is nearer than the title and would otherwise clip a two-line title (PT's "painel da reconstrução").
  - **Environment**: cream floor, fog and clear colour all `#F5F2EC`, so there is no horizon and the floor reads only through the per-card blurred blob shadows (tinted by `accentDeepLargeFor`, the decorative channel — the deep channel's yellow slot is an `rgba()` string `THREE.Color` cannot take). One tricolor halo per card via `accentFor`. Rounded corners are cut by GEOMETRY, never an alpha mask, so depth stays honest behind the halo.
  - **The breath**: cards bob, yaw and pitch on unrelated periods with per-card phase; scroll velocity feeds an energy accumulator that scales those amplitudes and leans the corridor against the direction of travel; the settled card tilts ≤ 6° toward a fine pointer and the title parallaxes a quarter of the way against it. All off under reduced motion, which keeps the pin and swaps cards instantly.
